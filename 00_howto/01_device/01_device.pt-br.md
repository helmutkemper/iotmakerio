...
Basta que você permita, compartilhando o seu código com a comunidade.
...
vai ler o código. 
Na seção 1.8 vamos ver como escrever a documentação em vários idiomas.
...
2. **A porta `value`** aparece com o tipo correto ao lado (`int`,
   `float32`, `string`).
3. **A cor do pino** indica à obrigatoriedade — a legenda abaixo dos
...
comentário do código-fonte — que é melhor que nada, mas mistura
...
No menu `export` > `image`, você pode salvar uma imagem do seu código, e se esta imagem não tiver sido editada, ela carrega uma copia do código fonte, e basta arrastar ela para o stage da IDE.

Mas, ela não carrega os devices que você criou, eles necessitam existir na biblioteca do usuário.
