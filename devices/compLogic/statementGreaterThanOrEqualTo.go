// devices/compLogic/statementGreaterThanOrEqualTo.go
//
...
import (
	...
	"github.com/helmutkemper/iotmakerio/ornament/compare"
...
type StatementGreaterThanOrEqualTo struct {
	...
	ornamentDraw *compare.OrnamentGreaterThanOrEqualTo
...
func (e *StatementGreaterThanOrEqualTo) SetStage(stage sprite.Stage) {
	e.stage = stage
}
...
func (e *StatementGreaterThanOrEqualTo) SetWireManager(mgr *wire.Manager) {
	e.wireMgr = mgr
}

// SetCanvasEl stores the canvas DOM element for positioning overlay inputs.
func (e *StatementGreaterThanOrEqualTo) SetCanvasEl(el js.Value) {
	e.canvasEl = el
}

// GetLabel returns the current editable label. Implements scene.Labeled.
func (e *StatementGreaterThanOrEqualTo) GetLabel() string { return e.label }

// SetLabel updates the label and re-renders the SVG.
func (e *StatementGreaterThanOrEqualTo) SetLabel(label string) {
	e.label = label
	go e.recacheOrnament()
}

// GetComment returns the user comment for code generation.
func (e *StatementGreaterThanOrEqualTo) GetComment() string { return e.comment }

// SetComment sets the comment that appears in generated code.
func (e *StatementGreaterThanOrEqualTo) SetComment(c string) { e.comment = c }

// GetDataType returns the current data type (int, float, string).
func (e *StatementGreaterThanOrEqualTo) GetDataType() string { return e.dataType }
...
func (e *StatementGreaterThanOrEqualTo) SetDataType(dt string) {
	if dt != "int" && dt != "float" && dt != "string" {
		log.Printf("[GreaterThanOrEqualTo] %s: invalid dataType %q, keeping %q", e.id, dt, e.dataType)
		return
	}
	if dt == e.dataType {
		return
	...
	// [PIN] repaint the connector pins: operand inputs take the new type's
	// canonical color; the OUTPUT keeps the bool color (a comparison result
	// is always bool — see updateConnectorTypes). Re-cache so the change is
	// visible immediately.
	// Português: Repinta os pinos: as entradas (operandos) recebem a cor
	// canônica do novo tipo; a SAÍDA mantém a cor de bool (o resultado de
	// uma comparação é sempre bool — ver updateConnectorTypes). Re-cacheia
	// para a mudança aparecer na hora.
	if e.ornamentDraw != nil {
		e.ornamentDraw.SetConnectionTypes(dt, dt, e.dataTypeOutput)
		go e.recacheOrnament()
	}
	log.Printf("[GreaterThanOrEqualTo] %s: dataType changed from %q to %q", e.id, old, dt)
}
...
func (e *StatementGreaterThanOrEqualTo) updateConnectorTypes() {
	if e.wireMgr == nil {
		return
	}
...
func (e *StatementGreaterThanOrEqualTo) IsHidden() bool { return e.hidden }

// SetHidden sets whether the label is hidden below the device.
func (e *StatementGreaterThanOrEqualTo) SetHidden(h bool) {
	e.hidden = h
	go func() {
...
func (e *StatementGreaterThanOrEqualTo) Append() {
...
func (e *StatementGreaterThanOrEqualTo) Remove() {
...
func (e *StatementGreaterThanOrEqualTo) SetOnRemove(fn func(id string)) {
	e.onRemove = fn
}

func (e *StatementGreaterThanOrEqualTo) SetResizerButton(resizeButton block.ResizeButton) {
	// [SPRITE] replaced: e.block.SetResizerButton(resizeButton)
	e.resizerButton = resizeButton
}

func (e *StatementGreaterThanOrEqualTo) SetGridAdjust(gridAdjust grid.Adjust) {
	e.gridAdjust = gridAdjust
}

func (e *StatementGreaterThanOrEqualTo) GetWidth() (width rulesDensity.Density) {
	if e.elem != nil {
...
func (e *StatementGreaterThanOrEqualTo) GetHeight() (height rulesDensity.Density) {
	if e.elem != nil {
...
func (e *StatementGreaterThanOrEqualTo) SetWarning(warning bool) {
	if !e.initialized {
		return
	}
...
func (e *StatementGreaterThanOrEqualTo) SetName(name string) {
	e.name = rulesSequentialId.GetIdFromBase(name)
}

func (e *StatementGreaterThanOrEqualTo) SetPosition(x, y rulesDensity.Density) {
	if e.elem != nil {
...
func (e *StatementGreaterThanOrEqualTo) SetSize(width, height rulesDensity.Density) {
	e.width = width
	e.height = height
	if e.elem != nil {
...
func (e *StatementGreaterThanOrEqualTo) SetHexMenu(m *mainMenu.SpriteHexMenu) {
	e.hexMenu = m
}

// SetContextMenu injects the shared linear context menu
// controller. Must be called before Init().
//
// Português: Injeta o controller do menu de contexto linear.
func (e *StatementGreaterThanOrEqualTo) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}

// getBodyMenuItems returns context menu items for a click on the body.
// Delete first (canonical order per D4), Inspect second, Settings last.
//
// Português: Itens do menu de contexto para clique no corpo.
// Ordem canonizada: Delete, Inspect, Settings.
func (e *StatementGreaterThanOrEqualTo) getBodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		...
		}),
		{
			...
			ViewBox:         "0 0 512 512",
			HelpKey:         "helpMenuGreaterThanOrEqualToSettings",
			HelpFallback:    "Changes the data type of every connector at once. Choose Int, Float, or String — the inputs and the output switch together so the operator keeps its signature consistent.",
			Submenu:         e.settingsSubmenuItems(),
		},
	}
}

// settingsSubmenuItems returns the type-picker submenu. The linear
// renderer injects its own Back row — submenus do NOT include a
// manual back button.
func (e *StatementGreaterThanOrEqualTo) settingsSubmenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		{
		...
		{
			ID: "typeString", Label: translate.T("menuTypeString", "String"),
			FontAwesomePath: rulesIcon.KFABars, ViewBox: "0 0 448 512",
			HelpKey:      "helpMenuTypeString",
			HelpFallback: "Sets every connector to string.",
			OnClick:      func() { e.SetDataType("string") },
		},
	}
}

// getInputXMenuItems returns the linear menu for the inputX connector.
func (e *StatementGreaterThanOrEqualTo) getInputXMenuItems() []contextMenu.Item {
	return mainMenu.ConnectorMenu(e.wireMgr, e.id, "inputX")
}

// getInputYMenuItems returns the linear menu for the inputY connector.
func (e *StatementGreaterThanOrEqualTo) getInputYMenuItems() []contextMenu.Item {
	return mainMenu.ConnectorMenu(e.wireMgr, e.id, "inputY")
}

// getOutputMenuItems returns the output-connector menu: Connect +
// Monitor submenu. The Monitor tree is preserved from the original
// hex version — the linear renderer handles depth via push-replace.
//
// Português: Menu do conector de saída: Connect + submenu Monitor. A
// árvore Monitor vem da versão hexagonal original; o renderer linear
// lida com a profundidade via push-replace.
func (e *StatementGreaterThanOrEqualTo) getOutputMenuItems() []contextMenu.Item {
...
func (e *StatementGreaterThanOrEqualTo) getOutputMonitorSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementGreaterThanOrEqualTo) getOutputValueSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementGreaterThanOrEqualTo) getOutputFormatSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementGreaterThanOrEqualTo) Init() (err error) {
	...
	e.dataTypeOutput = "bool"

	e.resizeLocked = true

	e.ornamentDraw = new(compare.OrnamentGreaterThanOrEqualTo)
	...
	// [PIN] paint the connector pins before the first Update/cache: operand
	// inputs use the initial data type's color, the output uses bool — the
	// comparison result type — so the pins match the wires from the first
	// render.
	// Português: Pinta os pinos antes do primeiro Update/cache: entradas com
	// a cor do tipo inicial, saída com bool — o tipo do resultado da
	// comparação — para os pinos casarem com os fios desde o primeiro
	// render.
	e.ornamentDraw.SetConnectionTypes(e.dataType, e.dataType, e.dataTypeOutput)
...
func (e *StatementGreaterThanOrEqualTo) wireEvents() {
...
// RegisterConnectors registers all connection points of this StatementGreaterThanOrEqualTo
// with the wire.Manager. Must be called AFTER Init() (when e.elem exists).
//
// Português:
//
//	RegisterConnectors registra todos os pontos de conexão deste StatementGreaterThanOrEqualTo
//	no wire.Manager. Deve ser chamado APÓS Init() (quando e.elem existe).
func (e *StatementGreaterThanOrEqualTo) RegisterConnectors() {
	...
	// outer tip so the wire leaves from OUTSIDE the device silhouette. The
	// type is dataTypeOutput (bool): a comparison result, never the operand
	// type.
	// Português: output — lado direito, no vértice do triângulo; ancorado na
	// ponta externa para o fio sair de FORA da silhueta do device. O tipo é
	// dataTypeOutput (bool): resultado de comparação, nunca o tipo dos
	// operandos.
	e.wireMgr.RegisterConnector(wire.ConnectorInfo{
		ID:                 wire.ConnectorID{ElementID: e.id, PortName: "output"},
		IsOutput:           true,
		AllowedTypes:       []string{e.dataTypeOutput},
...
func (e *StatementGreaterThanOrEqualTo) recacheOrnament() {
...
func (e *StatementGreaterThanOrEqualTo) injectLabelIntoSvg(svgXml string, ornH rulesDensity.Density) string {
...
func (e *StatementGreaterThanOrEqualTo) initWarningElement() {
...
func (e *StatementGreaterThanOrEqualTo) updateWarningPosition() {
...
func (e *StatementGreaterThanOrEqualTo) GetID() (id string) {
	return e.id
}

func (e *StatementGreaterThanOrEqualTo) SetSelected(selected bool) {
	if e.selectLocked {
		e.selected = false
		return
	}
...
func (e *StatementGreaterThanOrEqualTo) SetDragEnable(enabled bool) {
	if e.dragLocked {
		e.dragEnabled = false
		return
	}
...
func (e *StatementGreaterThanOrEqualTo) GetResizeEnable() (enabled bool) {
	if e.elem != nil {
...
func (e *StatementGreaterThanOrEqualTo) SetResizeEnable(enabled bool) {
	if e.resizeLocked {
		if e.elem != nil {
			e.elem.SetResizeEnable(false)
			e.elem.ShowResizeButtons(false)
...
func (e *StatementGreaterThanOrEqualTo) showInspectOverlay() {
...
func (e *StatementGreaterThanOrEqualTo) GetInspectConfig() interface{} {
	hiddenVal := "false"
	if e.hidden {
		hiddenVal = "true"
	}
	...
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
				...
				Fields: []overlay.Field{
					...
					},
					{
						...
						Options: []overlay.Option{
							...
							{Value: "string", Label: "String (concatenation)"},
						},
					},
					{
						Key:         "comment",
			...
			},
			{
				...
				ContentURL: "/help/devices/logic/statementGreaterThanOrEqualTo.md", // todo: to rules
			},
			{
...
func (e *StatementGreaterThanOrEqualTo) codePreview() string {
	code := ""
	if e.comment != "" {
		// Multi-line comments
	...
	case "string":
		...
		code += fmt.Sprintf("%s = inputX + inputY // concatenation", e.id)
	default: // int
		code += fmt.Sprintf("%s := inputX + inputY", e.id)
	}
	return code
...
// Implements scene.Inspectable.
func (e *StatementGreaterThanOrEqualTo) ApplyProperties(values map[string]string) {
	changed := false

	if v, ok := values["label"]; ok && v != "" && v != e.label {
		e.label = v
		changed = true
		log.Printf("[GreaterThanOrEqualTo] %s: label set to %q", e.id, v)
	}

	if v, ok := values["comment"]; ok && v != e.comment {
		e.comment = v
		log.Printf("[GreaterThanOrEqualTo] %s: comment set to %q", e.id, v)
	}
	...
	if v, ok := values["hidden"]; ok {
		...
		if h != e.hidden {
			e.SetHidden(h)
			log.Printf("[GreaterThanOrEqualTo] %s: hidden set to %v", e.id, h)
		}
	}
...
func (e *StatementGreaterThanOrEqualTo) GetProperties() map[string]interface{} {
...
func (e *StatementGreaterThanOrEqualTo) GetDeviceType() string {
	return "StatementGreaterThanOrEqualTo"
}

func (e *StatementGreaterThanOrEqualTo) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
// GetInnerBBox returns the usable area inside the ornament border.
// Math devices: 5px padding from outer edge (ornament border width).
func (e *StatementGreaterThanOrEqualTo) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementGreaterThanOrEqualTo) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementGreaterThanOrEqualTo) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// MoveBy shifts the device by (dx, dy) and updates all side effects.
// Implements scene.Movable — used by container drag logic.
func (e *StatementGreaterThanOrEqualTo) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementGreaterThanOrEqualTo) SetSceneMgr(mgr *scene.Serializer) { e.sceneMgr = mgr }
...
func (e *StatementGreaterThanOrEqualTo) OpenInspect() { go e.showInspectOverlay() }
