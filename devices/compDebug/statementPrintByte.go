// devices/compDebug/statementPrintByte.go
// SPDX-FileCopyrightText: 2026 Helmut Kemper
// SPDX-License-Identifier: AGPL-3.0-only

package compDebug

// statementPrintByte.go — Debug-print sink device (byte).
//
...
//	├─────────────────────┤  ← divider
//	│                     │
//	│   ◉   reg           │  ← prefix (or "print"), 16px bold, centered; ◉ = input pin
//	│                     │
//	└─────────────────────┘
//	printByte1               ← editable label, 12px muted
//
...
// StatementPrintByte is a debug-print sink device for type byte.
...
// optional text prefix and a format variant (hex or decimal).
//
...
// variante de formato.
type StatementPrintByte struct {
	stage sprite.Stage
	elem  sprite.Element
	...
	// format selects the per-type variant — for byte: "hex" | "decimal".
	...
	// format escolhe a variante por tipo — para byte: "hex" | "decimal". Os
...
func (e *StatementPrintByte) SetStage(s sprite.Stage)               { e.stage = s }
func (e *StatementPrintByte) SetWireManager(m *wire.Manager)        { e.wireMgr = m }
func (e *StatementPrintByte) SetResizerButton(r block.ResizeButton) { e.resizerButton = r }
func (e *StatementPrintByte) SetDraggerButton(_ block.ResizeButton) {}
func (e *StatementPrintByte) SetGridAdjust(g grid.Adjust)           { e.gridAdjust = g }

// SetContextMenu injects the linear context menu controller.
// Português: Injeta o controller do menu de contexto linear.
func (e *StatementPrintByte) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}
func (e *StatementPrintByte) SetPrefix(v string) {
	e.prefix = v
	if e.initialized {
		go e.recacheSVG()
	}
}
func (e *StatementPrintByte) GetPrefix() string { return e.prefix }
func (e *StatementPrintByte) SetFormat(v string) {
	e.format = v
}
func (e *StatementPrintByte) GetFormat() string              { return e.format }
func (e *StatementPrintByte) SetOnRemove(fn func(id string)) { e.onRemove = fn }

// ── Lifecycle ─────────────────────────────────────────────────────────────────

func (e *StatementPrintByte) Append() {
	if e.elem != nil {
		e.elem.SetVisible(true)
	}
}

func (e *StatementPrintByte) Remove() {
	if e.onRemove != nil {
		e.onRemove(e.id)
	}
	if e.wireMgr != nil {
...
func (e *StatementPrintByte) renderSVG() string {
	w := e.width.GetFloat()
...
func (e *StatementPrintByte) recacheSVG() {
	if e.elem != nil {
...
func (e *StatementPrintByte) Init() (err error) {
	if e.stage == nil {
		return fmt.Errorf("stage not set")
	}
	e.id = rulesSequentialId.GetIdFromBase("printByte")
	...
	if e.ctxMenu == nil {
		log.Printf("[PrintByte] warning: no context menu set — menus disabled")
	}
	e.wireEvents()
	e.initialized = true
...
func (e *StatementPrintByte) wireEvents() {
...
func (e *StatementPrintByte) bodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		mainMenu.DeleteItem(func() { log.Printf("[PrintByte] delete: %s", e.id); e.Remove() }),
...
func (e *StatementPrintByte) showInspectOverlay() { overlay.Show(e.inspectConfig()) }

func (e *StatementPrintByte) inspectConfig() overlay.Config {
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
			...
			{Label: "Help", Type: overlay.TabMarkdown, Content: printByteHelp()},
		},
...
func (e *StatementPrintByte) GetInspectConfig() interface{} { return e.inspectConfig() }
func (e *StatementPrintByte) ApplyProperties(values map[string]string) {
	if v, ok := values["comment"]; ok {
		e.comment = v
	}
	if v, ok := values["prefix"]; ok {
...
func (e *StatementPrintByte) RegisterConnectors() {
	...
	e.wireMgr.RegisterConnector(wire.ConnectorInfo{
		...
		IsOutput:           false,
		AllowedTypes:       []string{"byte", "uint8", "byte*"},
...
func (e *StatementPrintByte) SetName(n string)  { e.name = rulesSequentialId.GetIdFromBase(n) }
func (e *StatementPrintByte) Get() *html.TagSvg { return nil }
func (e *StatementPrintByte) SetPosition(x, y rulesDensity.Density) {
	if e.elem != nil {
...
// a faixa do label).
func (e *StatementPrintByte) SetSize(w, _ rulesDensity.Density) {
...
func (e *StatementPrintByte) GetWidth() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetWidthD()
	}
	return e.width
}
func (e *StatementPrintByte) GetHeight() rulesDensity.Density { return e.height }
func (e *StatementPrintByte) GetX() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetXD()
	}
	return 0
}
func (e *StatementPrintByte) GetY() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetYD()
	}
	return 0
}
func (e *StatementPrintByte) SetX(x rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(x, e.elem.GetYD())
	}
}
func (e *StatementPrintByte) SetY(y rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(e.elem.GetXD(), y)
	}
}
func (e *StatementPrintByte) SetWidth(w rulesDensity.Density) {
	e.width = w
	if e.elem != nil {
		e.elem.SetSizeD(w, e.height+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintByte) SetHeight(h rulesDensity.Density) {
	e.height = h
	if e.elem != nil {
		e.elem.SetSizeD(e.elem.GetWidthD(), h+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintByte) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
...
func (e *StatementPrintByte) GetInitialized() bool   { return e.initialized }
func (e *StatementPrintByte) GetID() string          { return e.id }
func (e *StatementPrintByte) GetName() string        { return e.name }
func (e *StatementPrintByte) GetSelected() bool      { return e.selected }
func (e *StatementPrintByte) GetSelectBlocked() bool { return e.selectLocked }
func (e *StatementPrintByte) GetDragEnable() bool    { return e.dragEnabled }
func (e *StatementPrintByte) GetDragBlocked() bool   { return e.dragLocked }
func (e *StatementPrintByte) GetResizeEnable() bool {
	if e.elem != nil {
		return e.elem.IsResizeEnabled()
	}
	return false
}
func (e *StatementPrintByte) GetResize() bool        { return e.GetResizeEnable() }
func (e *StatementPrintByte) GetResizeBlocked() bool { return e.resizeLocked }
func (e *StatementPrintByte) GetZIndex() int {
	if e.elem != nil {
		return e.elem.GetIndex()
	}
	return 0
}
func (e *StatementPrintByte) GetStatus() int  { return e.iconStatus }
func (e *StatementPrintByte) SetStatus(s int) { e.iconStatus = s }
func (e *StatementPrintByte) SelectedInvert() { e.SetSelected(!e.selected) }

func (e *StatementPrintByte) SetSelected(sel bool) {
	if e.selectLocked {
		e.selected = false
		return
	}
...
func (e *StatementPrintByte) SetDragEnable(en bool) {
	if e.dragLocked {
		e.dragEnabled = false
		return
	}
...
func (e *StatementPrintByte) SetResizeEnable(enabled bool) {
	if e.elem == nil {
...
func (e *StatementPrintByte) GetIconName() string     { return "PrintByte" }
func (e *StatementPrintByte) GetIconCategory() string { return "Debug" }

func (e *StatementPrintByte) getIcon(data rulesIcon.Data) js.Value {
...
func (e *StatementPrintByte) GetDeviceType() string { return "StatementPrintByte" }
func (e *StatementPrintByte) GetProperties() map[string]interface{} {
...
func (e *StatementPrintByte) GetComment() string { return e.comment }

// SetComment sets the user comment.
// Português: Define o comentário do usuário.
func (e *StatementPrintByte) SetComment(c string) { e.comment = c }
...
func (e *StatementPrintByte) codePreview() string {
	verb := "0x%02X"
	if e.format == "decimal" {
		verb = "%d"
	}
...
func (e *StatementPrintByte) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintByte) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintByte) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementPrintByte) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// ── Help text ─────────────────────────────────────────────────────────────────

func printByteHelp() string {
	return `# PrintByte — Print Byte
...
This device is the stage's **universal probe**: besides plain ` + "`byte`" + `
wires it also accepts ` + "`byte*`" + ` — the DASHED wires produced by
...
| Format   | select | Hexadecimal (0x2A) or Decimal (42)             |
...
- Hexadecimal is the natural notation for protocol and register work; the
  ` + "`0x%02X`" + ` form always prints two digits (` + "`0x0A`" + `, not ` + "`0xA`" + `).
`
}
...
func (e *StatementPrintByte) SetSceneMgr(mgr *scene.Serializer) { e.sceneMgr = mgr }
...
func (e *StatementPrintByte) OpenInspect() { go e.showInspectOverlay() }
