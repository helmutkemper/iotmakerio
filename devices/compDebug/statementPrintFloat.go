// devices/compDebug/statementPrintFloat.go
// SPDX-FileCopyrightText: 2026 Helmut Kemper
// SPDX-License-Identifier: AGPL-3.0-only

package compDebug

// statementPrintFloat.go — Debug-print sink device (float).
//
...
//	│ DEBUG           float │  ← 18px header: "PRINT" tag (left), type label (right)
//	├─────────────────────┤  ← divider
//	│                     │
//	│   ◉   sensor        │  ← prefix (or "print"), 16px bold, centered; ◉ = input pin
//	│                     │
//	└─────────────────────┘
//	printFloat1               ← editable label, 12px muted
//
// Body click:      Inspect · Delete
// Connector click: Connect (input-only)
// Double-click:    Inspect overlay
//
// Português: Device sink "imprimir" (float). Imprime o valor ligado na entrada
...
// StatementPrintFloat is a debug-print sink device for type float.
...
// optional text prefix and a format variant (natural or truncated).
//
// Português: Device sink de print de depuração para float. Conector único
...
// variante de formato.
type StatementPrintFloat struct {
	stage sprite.Stage
	elem  sprite.Element
	...
	// format selects the per-type variant — for float: "float" | "trunc" (integer
	// part, truncated toward zero, never rounded).
	...
	// format escolhe a variante por tipo — para float: "float" | "trunc" (parte
	// inteira, truncada, nunca arredondada). Os
...
func (e *StatementPrintFloat) SetStage(s sprite.Stage)               { e.stage = s }
func (e *StatementPrintFloat) SetWireManager(m *wire.Manager)        { e.wireMgr = m }
func (e *StatementPrintFloat) SetResizerButton(r block.ResizeButton) { e.resizerButton = r }
func (e *StatementPrintFloat) SetDraggerButton(_ block.ResizeButton) {}
func (e *StatementPrintFloat) SetGridAdjust(g grid.Adjust)           { e.gridAdjust = g }

// SetContextMenu injects the linear context menu controller.
// Português: Injeta o controller do menu de contexto linear.
func (e *StatementPrintFloat) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}
func (e *StatementPrintFloat) SetPrefix(v string) {
	e.prefix = v
	if e.initialized {
		go e.recacheSVG()
	}
}
func (e *StatementPrintFloat) GetPrefix() string { return e.prefix }
func (e *StatementPrintFloat) SetFormat(v string) {
	e.format = v
}
func (e *StatementPrintFloat) GetFormat() string              { return e.format }
func (e *StatementPrintFloat) SetOnRemove(fn func(id string)) { e.onRemove = fn }

// ── Lifecycle ─────────────────────────────────────────────────────────────────

func (e *StatementPrintFloat) Append() {
	if e.elem != nil {
		e.elem.SetVisible(true)
	}
}

func (e *StatementPrintFloat) Remove() {
	if e.onRemove != nil {
		e.onRemove(e.id)
	}
	if e.wireMgr != nil {
...
func (e *StatementPrintFloat) renderSVG() string {
	w := e.width.GetFloat()
...
func (e *StatementPrintFloat) recacheSVG() {
	if e.elem != nil {
...
func (e *StatementPrintFloat) Init() (err error) {
	if e.stage == nil {
		return fmt.Errorf("stage not set")
	}
	e.id = rulesSequentialId.GetIdFromBase("printFloat")
	...
	if e.ctxMenu == nil {
		log.Printf("[PrintFloat] warning: no context menu set — menus disabled")
	}
	e.wireEvents()
	e.initialized = true
...
func (e *StatementPrintFloat) wireEvents() {
...
func (e *StatementPrintFloat) bodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		mainMenu.DeleteItem(func() { log.Printf("[PrintFloat] delete: %s", e.id); e.Remove() }),
...
func (e *StatementPrintFloat) showInspectOverlay() { overlay.Show(e.inspectConfig()) }

func (e *StatementPrintFloat) inspectConfig() overlay.Config {
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
				...
				Fields: []overlay.Field{
					{Key: "prefix", Label: translate.T("propPrintPrefix", "Prefix"), Type: overlay.FieldText, Value: e.prefix, Placeholder: "temp"},
					{
						...
						Options: []overlay.Option{
							{Value: "float", Label: "Float (3.14)"},
							{Value: "trunc", Label: "Truncated integer (3)"},
						},
					},
			...
			{Label: "Help", Type: overlay.TabMarkdown, Content: printFloatHelp()},
		},
...
func (e *StatementPrintFloat) GetInspectConfig() interface{} { return e.inspectConfig() }
func (e *StatementPrintFloat) ApplyProperties(values map[string]string) {
	if v, ok := values["comment"]; ok {
		e.comment = v
	}
	if v, ok := values["prefix"]; ok {
...
func (e *StatementPrintFloat) RegisterConnectors() {
	...
	e.wireMgr.RegisterConnector(wire.ConnectorInfo{
		...
		IsOutput:           false,
		AllowedTypes:       []string{"float", "float64", "float*"},
...
func (e *StatementPrintFloat) SetName(n string)  { e.name = rulesSequentialId.GetIdFromBase(n) }
func (e *StatementPrintFloat) Get() *html.TagSvg { return nil }
func (e *StatementPrintFloat) SetPosition(x, y rulesDensity.Density) {
	if e.elem != nil {
...
// a faixa do label).
func (e *StatementPrintFloat) SetSize(w, _ rulesDensity.Density) {
...
func (e *StatementPrintFloat) GetWidth() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetWidthD()
	}
	return e.width
}
func (e *StatementPrintFloat) GetHeight() rulesDensity.Density { return e.height }
func (e *StatementPrintFloat) GetX() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetXD()
	}
	return 0
}
func (e *StatementPrintFloat) GetY() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetYD()
	}
	return 0
}
func (e *StatementPrintFloat) SetX(x rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(x, e.elem.GetYD())
	}
}
func (e *StatementPrintFloat) SetY(y rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(e.elem.GetXD(), y)
	}
}
func (e *StatementPrintFloat) SetWidth(w rulesDensity.Density) {
	e.width = w
	if e.elem != nil {
		e.elem.SetSizeD(w, e.height+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintFloat) SetHeight(h rulesDensity.Density) {
	e.height = h
	if e.elem != nil {
		e.elem.SetSizeD(e.elem.GetWidthD(), h+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintFloat) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
...
func (e *StatementPrintFloat) GetInitialized() bool   { return e.initialized }
func (e *StatementPrintFloat) GetID() string          { return e.id }
func (e *StatementPrintFloat) GetName() string        { return e.name }
func (e *StatementPrintFloat) GetSelected() bool      { return e.selected }
func (e *StatementPrintFloat) GetSelectBlocked() bool { return e.selectLocked }
func (e *StatementPrintFloat) GetDragEnable() bool    { return e.dragEnabled }
func (e *StatementPrintFloat) GetDragBlocked() bool   { return e.dragLocked }
func (e *StatementPrintFloat) GetResizeEnable() bool {
	if e.elem != nil {
		return e.elem.IsResizeEnabled()
	}
	return false
}
func (e *StatementPrintFloat) GetResize() bool        { return e.GetResizeEnable() }
func (e *StatementPrintFloat) GetResizeBlocked() bool { return e.resizeLocked }
func (e *StatementPrintFloat) GetZIndex() int {
	if e.elem != nil {
		return e.elem.GetIndex()
	}
	return 0
}
func (e *StatementPrintFloat) GetStatus() int  { return e.iconStatus }
func (e *StatementPrintFloat) SetStatus(s int) { e.iconStatus = s }
func (e *StatementPrintFloat) SelectedInvert() { e.SetSelected(!e.selected) }

func (e *StatementPrintFloat) SetSelected(sel bool) {
	if e.selectLocked {
		e.selected = false
		return
	}
...
func (e *StatementPrintFloat) SetDragEnable(en bool) {
	if e.dragLocked {
		e.dragEnabled = false
		return
	}
...
func (e *StatementPrintFloat) SetResizeEnable(enabled bool) {
	if e.elem == nil {
...
func (e *StatementPrintFloat) GetIconName() string     { return "PrintFloat" }
func (e *StatementPrintFloat) GetIconCategory() string { return "Debug" }

func (e *StatementPrintFloat) getIcon(data rulesIcon.Data) js.Value {
...
func (e *StatementPrintFloat) GetDeviceType() string { return "StatementPrintFloat" }
func (e *StatementPrintFloat) GetProperties() map[string]interface{} {
...
func (e *StatementPrintFloat) GetComment() string { return e.comment }

// SetComment sets the user comment.
// Português: Define o comentário do usuário.
func (e *StatementPrintFloat) SetComment(c string) { e.comment = c }
...
func (e *StatementPrintFloat) codePreview() string {
	arg := "value"
	verb := "%v"
	if e.format == "trunc" {
		verb = "%d"
	...
	return devices.CommentPrefix(e.comment) + fmt.Sprintf("// Print (float)\nfmt.Printf(%q, %s)", lead+verb+"\n", arg)
}
func (e *StatementPrintFloat) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintFloat) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintFloat) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementPrintFloat) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// ── Help text ─────────────────────────────────────────────────────────────────

func printFloatHelp() string {
	return `# PrintFloat — Print Float
...
This device is the stage's **universal probe**: besides plain ` + "`float`" + `
wires it also accepts ` + "`float*`" + ` — the DASHED wires produced by
...
| Format   | select | Float (3.14) or Truncated integer (3, no rounding) |
...
- **Truncated** keeps the integer part as-is: 3.9 prints ` + "`3`" + `, never ` + "`4`" + `.
- **Arduino Uno (AVR) caveat**: avr-libc's printf ships WITHOUT float
  support unless the project links ` + "`-Wl,-u,vfprintf -lprintf_flt`" + ` —
  otherwise the Float format prints ` + "`?`" + `. The Truncated format works
  everywhere.
...
func (e *StatementPrintFloat) SetSceneMgr(mgr *scene.Serializer) { e.sceneMgr = mgr }
...
func (e *StatementPrintFloat) OpenInspect() { go e.showInspectOverlay() }
