// devices/compDebug/statementPrintByteArray.go
// SPDX-FileCopyrightText: 2026 Helmut Kemper
// SPDX-License-Identifier: AGPL-3.0-only

package compDebug

// statementPrintByteArray.go — Debug-print sink device ([]byte).
//
...
//	│ DEBUG          []byte │  ← 18px header: "PRINT" tag (left), type label (right)
//	├─────────────────────┤  ← divider
//	│                     │
//	│   ◉   rx            │  ← prefix (or "print"), 16px bold, centered; ◉ = input pin
//	│                     │
//	└─────────────────────┘
//	printByteArray1               ← editable label, 12px muted
//
// Body click:      Inspect · Delete
// Connector click: Connect (input-only)
// Double-click:    Inspect overlay
//
// Português: Device sink "imprimir" ([]byte). Imprime o valor ligado na entrada
...
// StatementPrintByteArray is a debug-print sink device for type []byte.
...
// optional text prefix and a format variant (hex pairs or text).
//
// Português: Device sink de print de depuração para []byte. Conector único
...
// variante de formato.
type StatementPrintByteArray struct {
	stage sprite.Stage
	elem  sprite.Element
	...
	// format selects the per-type variant — for []byte: "hex" (space-separated
	// pairs) | "text" (the bytes as UTF-8).
	...
	// format escolhe a variante por tipo — para []byte: "hex" (pares separados
	// por espaço) | "text" (os bytes como UTF-8). Os
...
func (e *StatementPrintByteArray) SetStage(s sprite.Stage)               { e.stage = s }
func (e *StatementPrintByteArray) SetWireManager(m *wire.Manager)        { e.wireMgr = m }
func (e *StatementPrintByteArray) SetResizerButton(r block.ResizeButton) { e.resizerButton = r }
func (e *StatementPrintByteArray) SetDraggerButton(_ block.ResizeButton) {}
func (e *StatementPrintByteArray) SetGridAdjust(g grid.Adjust)           { e.gridAdjust = g }

// SetContextMenu injects the linear context menu controller.
// Português: Injeta o controller do menu de contexto linear.
func (e *StatementPrintByteArray) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}
func (e *StatementPrintByteArray) SetPrefix(v string) {
	e.prefix = v
	if e.initialized {
		go e.recacheSVG()
	}
}
func (e *StatementPrintByteArray) GetPrefix() string { return e.prefix }
func (e *StatementPrintByteArray) SetFormat(v string) {
	e.format = v
}
func (e *StatementPrintByteArray) GetFormat() string              { return e.format }
func (e *StatementPrintByteArray) SetOnRemove(fn func(id string)) { e.onRemove = fn }

// ── Lifecycle ─────────────────────────────────────────────────────────────────

func (e *StatementPrintByteArray) Append() {
	if e.elem != nil {
		e.elem.SetVisible(true)
	}
}

func (e *StatementPrintByteArray) Remove() {
	if e.onRemove != nil {
		e.onRemove(e.id)
	}
	if e.wireMgr != nil {
...
func (e *StatementPrintByteArray) renderSVG() string {
	w := e.width.GetFloat()
	...
	ts := rulesDevice.TypeStyleFor("[]byte")
	...
	svg += fmt.Sprintf(
		`<text x="%.1f" y="%.1f" font-family="%s" font-size="%d" fill="%s" text-anchor="end" dominant-baseline="middle">[]byte</text>`,
...
func (e *StatementPrintByteArray) recacheSVG() {
	if e.elem != nil {
...
func (e *StatementPrintByteArray) Init() (err error) {
	if e.stage == nil {
		return fmt.Errorf("stage not set")
	}
	e.id = rulesSequentialId.GetIdFromBase("printByteArray")
	...
	if e.ctxMenu == nil {
		log.Printf("[PrintByteArray] warning: no context menu set — menus disabled")
	}
	e.wireEvents()
	e.initialized = true
...
func (e *StatementPrintByteArray) wireEvents() {
...
func (e *StatementPrintByteArray) bodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		mainMenu.DeleteItem(func() { log.Printf("[PrintByteArray] delete: %s", e.id); e.Remove() }),
...
func (e *StatementPrintByteArray) showInspectOverlay() { overlay.Show(e.inspectConfig()) }

func (e *StatementPrintByteArray) inspectConfig() overlay.Config {
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
				...
				Fields: []overlay.Field{
					{Key: "prefix", Label: translate.T("propPrintPrefix", "Prefix"), Type: overlay.FieldText, Value: e.prefix, Placeholder: "temp"},
					{
						...
						Options: []overlay.Option{
							{Value: "hex", Label: "Hex pairs (41 42 43)"},
			...
			{Label: "Help", Type: overlay.TabMarkdown, Content: printByteArrayHelp()},
		},
...
func (e *StatementPrintByteArray) GetInspectConfig() interface{} { return e.inspectConfig() }
func (e *StatementPrintByteArray) ApplyProperties(values map[string]string) {
	if v, ok := values["comment"]; ok {
		e.comment = v
	}
	if v, ok := values["prefix"]; ok {
...
func (e *StatementPrintByteArray) RegisterConnectors() {
...
func (e *StatementPrintByteArray) SetName(n string)  { e.name = rulesSequentialId.GetIdFromBase(n) }
func (e *StatementPrintByteArray) Get() *html.TagSvg { return nil }
func (e *StatementPrintByteArray) SetPosition(x, y rulesDensity.Density) {
	if e.elem != nil {
...
// a faixa do label).
func (e *StatementPrintByteArray) SetSize(w, _ rulesDensity.Density) {
...
func (e *StatementPrintByteArray) GetWidth() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetWidthD()
	}
	return e.width
}
func (e *StatementPrintByteArray) GetHeight() rulesDensity.Density { return e.height }
func (e *StatementPrintByteArray) GetX() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetXD()
	}
	return 0
}
func (e *StatementPrintByteArray) GetY() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetYD()
	}
	return 0
}
func (e *StatementPrintByteArray) SetX(x rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(x, e.elem.GetYD())
	}
}
func (e *StatementPrintByteArray) SetY(y rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(e.elem.GetXD(), y)
	}
}
func (e *StatementPrintByteArray) SetWidth(w rulesDensity.Density) {
	e.width = w
	if e.elem != nil {
		e.elem.SetSizeD(w, e.height+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintByteArray) SetHeight(h rulesDensity.Density) {
	e.height = h
	if e.elem != nil {
		e.elem.SetSizeD(e.elem.GetWidthD(), h+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintByteArray) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
...
func (e *StatementPrintByteArray) GetInitialized() bool   { return e.initialized }
func (e *StatementPrintByteArray) GetID() string          { return e.id }
func (e *StatementPrintByteArray) GetName() string        { return e.name }
func (e *StatementPrintByteArray) GetSelected() bool      { return e.selected }
func (e *StatementPrintByteArray) GetSelectBlocked() bool { return e.selectLocked }
func (e *StatementPrintByteArray) GetDragEnable() bool    { return e.dragEnabled }
func (e *StatementPrintByteArray) GetDragBlocked() bool   { return e.dragLocked }
func (e *StatementPrintByteArray) GetResizeEnable() bool {
	if e.elem != nil {
		return e.elem.IsResizeEnabled()
	}
	return false
}
func (e *StatementPrintByteArray) GetResize() bool        { return e.GetResizeEnable() }
func (e *StatementPrintByteArray) GetResizeBlocked() bool { return e.resizeLocked }
func (e *StatementPrintByteArray) GetZIndex() int {
	if e.elem != nil {
		return e.elem.GetIndex()
	}
	return 0
}
func (e *StatementPrintByteArray) GetStatus() int  { return e.iconStatus }
func (e *StatementPrintByteArray) SetStatus(s int) { e.iconStatus = s }
func (e *StatementPrintByteArray) SelectedInvert() { e.SetSelected(!e.selected) }

func (e *StatementPrintByteArray) SetSelected(sel bool) {
	if e.selectLocked {
		e.selected = false
		return
	}
...
func (e *StatementPrintByteArray) SetDragEnable(en bool) {
	if e.dragLocked {
		e.dragEnabled = false
		return
	}
...
func (e *StatementPrintByteArray) SetResizeEnable(enabled bool) {
	if e.elem == nil {
...
func (e *StatementPrintByteArray) GetIconName() string     { return "PrintByteArray" }
func (e *StatementPrintByteArray) GetIconCategory() string { return "Debug" }

func (e *StatementPrintByteArray) getIcon(data rulesIcon.Data) js.Value {
...
func (e *StatementPrintByteArray) GetDeviceType() string { return "StatementPrintByteArray" }
func (e *StatementPrintByteArray) GetProperties() map[string]interface{} {
...
func (e *StatementPrintByteArray) GetComment() string { return e.comment }

// SetComment sets the user comment.
// Português: Define o comentário do usuário.
func (e *StatementPrintByteArray) SetComment(c string) { e.comment = c }
...
func (e *StatementPrintByteArray) codePreview() string {
	verb := "% X"
	if e.format == "text" {
		verb = "%s"
	}
	...
	return devices.CommentPrefix(e.comment) + fmt.Sprintf("// Print ([]byte)\nfmt.Printf(%q, value)", lead+verb+"\n")
}
func (e *StatementPrintByteArray) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintByteArray) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintByteArray) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementPrintByteArray) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// ── Help text ─────────────────────────────────────────────────────────────────

func printByteArrayHelp() string {
	return `# PrintByteArray — Print Byte Slice

Prints the byte collection wired into its input to **standard output**, once
per execution of its scope (every loop turn when placed inside a loop).

At code generation it becomes ` + "`fmt.Printf`" + ` in Go and a ` + "`printf`" + ` loop in
C99 — this device ALWAYS lands in the generated program, unlike display
widgets.
...
| Format   | select | Hex pairs (41 42 43) or Text (the bytes as UTF-8) |
...
- **Hex pairs** is the wire-sniffer view; **Text** treats the bytes as a
  UTF-8 string (length-bounded — no terminator needed in C).
- In **C99** the collection must follow the ` + "`_len`" + ` companion convention
  (constant collections and library outputs already do).
`
}
...
func (e *StatementPrintByteArray) SetSceneMgr(mgr *scene.Serializer) { e.sceneMgr = mgr }
...
func (e *StatementPrintByteArray) OpenInspect() { go e.showInspectOverlay() }
