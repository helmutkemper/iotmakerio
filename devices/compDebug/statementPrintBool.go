// devices/compDebug/statementPrintBool.go
// SPDX-FileCopyrightText: 2026 Helmut Kemper
// SPDX-License-Identifier: AGPL-3.0-only

package compDebug

// statementPrintBool.go — Debug-print sink device (bool).
//
...
//	│ DEBUG            bool │  ← 18px header: "PRINT" tag (left), type label (right)
//	├─────────────────────┤  ← divider
//	│                     │
//	│   ◉   alarm         │  ← prefix (or "print"), 16px bold, centered; ◉ = input pin
//	│                     │
//	└─────────────────────┘
//	printBool1               ← editable label, 12px muted
//
// Body click:      Inspect · Delete
// Connector click: Connect (input-only)
// Double-click:    Inspect overlay
//
// Português: Device sink "imprimir" (bool). Imprime o valor ligado na entrada
...
// StatementPrintBool is a debug-print sink device for type bool.
...
// optional text prefix and a format variant (true/false or 1/0).
//
// Português: Device sink de print de depuração para bool. Conector único
...
// variante de formato.
type StatementPrintBool struct {
	stage sprite.Stage
	elem  sprite.Element
	...
	// format selects the per-type variant — for bool: "truefalse" | "onezero".
	...
	// format escolhe a variante por tipo — para bool: "truefalse" | "onezero". Os
...
func (e *StatementPrintBool) SetStage(s sprite.Stage)               { e.stage = s }
func (e *StatementPrintBool) SetWireManager(m *wire.Manager)        { e.wireMgr = m }
func (e *StatementPrintBool) SetResizerButton(r block.ResizeButton) { e.resizerButton = r }
func (e *StatementPrintBool) SetDraggerButton(_ block.ResizeButton) {}
func (e *StatementPrintBool) SetGridAdjust(g grid.Adjust)           { e.gridAdjust = g }

// SetContextMenu injects the linear context menu controller.
// Português: Injeta o controller do menu de contexto linear.
func (e *StatementPrintBool) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}
func (e *StatementPrintBool) SetPrefix(v string) {
	e.prefix = v
	if e.initialized {
		go e.recacheSVG()
	}
}
func (e *StatementPrintBool) GetPrefix() string { return e.prefix }
func (e *StatementPrintBool) SetFormat(v string) {
	e.format = v
}
func (e *StatementPrintBool) GetFormat() string              { return e.format }
func (e *StatementPrintBool) SetOnRemove(fn func(id string)) { e.onRemove = fn }

// ── Lifecycle ─────────────────────────────────────────────────────────────────

func (e *StatementPrintBool) Append() {
	if e.elem != nil {
		e.elem.SetVisible(true)
	}
}

func (e *StatementPrintBool) Remove() {
	if e.onRemove != nil {
		e.onRemove(e.id)
	}
	if e.wireMgr != nil {
...
func (e *StatementPrintBool) renderSVG() string {
	w := e.width.GetFloat()
	...
	svg += fmt.Sprintf(
		`<text x="%.1f" y="%.1f" font-family="%s" font-size="%d" fill="%s" text-anchor="end" dominant-baseline="middle">bool</text>`,
...
func (e *StatementPrintBool) recacheSVG() {
	if e.elem != nil {
...
func (e *StatementPrintBool) Init() (err error) {
	if e.stage == nil {
		return fmt.Errorf("stage not set")
	}
	e.id = rulesSequentialId.GetIdFromBase("printBool")
	...
	if e.ctxMenu == nil {
		log.Printf("[PrintBool] warning: no context menu set — menus disabled")
	}
	e.wireEvents()
	e.initialized = true
...
func (e *StatementPrintBool) wireEvents() {
...
func (e *StatementPrintBool) bodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		mainMenu.DeleteItem(func() { log.Printf("[PrintBool] delete: %s", e.id); e.Remove() }),
...
func (e *StatementPrintBool) showInspectOverlay() { overlay.Show(e.inspectConfig()) }

func (e *StatementPrintBool) inspectConfig() overlay.Config {
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
			...
			{Label: "Help", Type: overlay.TabMarkdown, Content: printBoolHelp()},
		},
...
func (e *StatementPrintBool) GetInspectConfig() interface{} { return e.inspectConfig() }
func (e *StatementPrintBool) ApplyProperties(values map[string]string) {
	if v, ok := values["comment"]; ok {
		e.comment = v
	}
	if v, ok := values["prefix"]; ok {
...
func (e *StatementPrintBool) RegisterConnectors() {
...
func (e *StatementPrintBool) SetName(n string)  { e.name = rulesSequentialId.GetIdFromBase(n) }
func (e *StatementPrintBool) Get() *html.TagSvg { return nil }
func (e *StatementPrintBool) SetPosition(x, y rulesDensity.Density) {
	if e.elem != nil {
...
// a faixa do label).
func (e *StatementPrintBool) SetSize(w, _ rulesDensity.Density) {
...
func (e *StatementPrintBool) GetWidth() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetWidthD()
	}
	return e.width
}
func (e *StatementPrintBool) GetHeight() rulesDensity.Density { return e.height }
func (e *StatementPrintBool) GetX() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetXD()
	}
	return 0
}
func (e *StatementPrintBool) GetY() rulesDensity.Density {
	if e.elem != nil {
		return e.elem.GetYD()
	}
	return 0
}
func (e *StatementPrintBool) SetX(x rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(x, e.elem.GetYD())
	}
}
func (e *StatementPrintBool) SetY(y rulesDensity.Density) {
	if e.elem != nil {
		e.elem.SetPositionD(e.elem.GetXD(), y)
	}
}
func (e *StatementPrintBool) SetWidth(w rulesDensity.Density) {
	e.width = w
	if e.elem != nil {
		e.elem.SetSizeD(w, e.height+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintBool) SetHeight(h rulesDensity.Density) {
	e.height = h
	if e.elem != nil {
		e.elem.SetSizeD(e.elem.GetWidthD(), h+rulesDevice.KLabelHeight)
	}
}
func (e *StatementPrintBool) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
...
func (e *StatementPrintBool) GetInitialized() bool   { return e.initialized }
func (e *StatementPrintBool) GetID() string          { return e.id }
func (e *StatementPrintBool) GetName() string        { return e.name }
func (e *StatementPrintBool) GetSelected() bool      { return e.selected }
func (e *StatementPrintBool) GetSelectBlocked() bool { return e.selectLocked }
func (e *StatementPrintBool) GetDragEnable() bool    { return e.dragEnabled }
func (e *StatementPrintBool) GetDragBlocked() bool   { return e.dragLocked }
func (e *StatementPrintBool) GetResizeEnable() bool {
	if e.elem != nil {
		return e.elem.IsResizeEnabled()
	}
	return false
}
func (e *StatementPrintBool) GetResize() bool        { return e.GetResizeEnable() }
func (e *StatementPrintBool) GetResizeBlocked() bool { return e.resizeLocked }
func (e *StatementPrintBool) GetZIndex() int {
	if e.elem != nil {
		return e.elem.GetIndex()
	}
	return 0
}
func (e *StatementPrintBool) GetStatus() int  { return e.iconStatus }
func (e *StatementPrintBool) SetStatus(s int) { e.iconStatus = s }
func (e *StatementPrintBool) SelectedInvert() { e.SetSelected(!e.selected) }

func (e *StatementPrintBool) SetSelected(sel bool) {
	if e.selectLocked {
		e.selected = false
		return
	}
...
func (e *StatementPrintBool) SetDragEnable(en bool) {
	if e.dragLocked {
		e.dragEnabled = false
		return
	}
...
func (e *StatementPrintBool) SetResizeEnable(enabled bool) {
	if e.elem == nil {
...
func (e *StatementPrintBool) GetIconName() string     { return "PrintBool" }
func (e *StatementPrintBool) GetIconCategory() string { return "Debug" }

func (e *StatementPrintBool) getIcon(data rulesIcon.Data) js.Value {
...
func (e *StatementPrintBool) GetDeviceType() string { return "StatementPrintBool" }
func (e *StatementPrintBool) GetProperties() map[string]interface{} {
...
func (e *StatementPrintBool) GetComment() string { return e.comment }

// SetComment sets the user comment.
// Português: Define o comentário do usuário.
func (e *StatementPrintBool) SetComment(c string) { e.comment = c }
...
func (e *StatementPrintBool) codePreview() string {
	lead := ""
	if e.prefix != "" {
		lead = e.prefix + " "
	}
	if e.format == "onezero" {
		return fmt.Sprintf("// Print (bool)\nvalueAsInt := 0\nif value {\n\tvalueAsInt = 1\n}\nfmt.Printf(%q, valueAsInt)", lead+"%d\n")
	}
	return devices.CommentPrefix(e.comment) + fmt.Sprintf("// Print (bool)\nfmt.Printf(%q, value)", lead+"%t\n")
}
func (e *StatementPrintBool) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintBool) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementPrintBool) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementPrintBool) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// ── Help text ─────────────────────────────────────────────────────────────────

func printBoolHelp() string {
	return `# PrintBool — Print Boolean
...
| Format   | select | true / false, or 1 / 0                         |
...
- ` + "`1 / 0`" + ` is handy when the output feeds a log parser or a spreadsheet.
- Wire a comparison device (EqualTo, LessThan, ...) straight into the input.
`
}
...
func (e *StatementPrintBool) SetSceneMgr(mgr *scene.Serializer) { e.sceneMgr = mgr }
...
func (e *StatementPrintBool) OpenInspect() { go e.showInspectOverlay() }
