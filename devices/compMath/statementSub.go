// devices/compMath/statementSub.go
//
...
func (e *StatementSub) SetStage(stage sprite.Stage) {
	e.stage = stage
}
...
func (e *StatementSub) SetDataType(dt string) {
	if dt != "int" && dt != "float" {
		log.Printf("[Sub] %s: invalid dataType %q, keeping %q", e.id, dt, e.dataType)
		return
	}
	if dt == e.dataType {
		return
	...
	log.Printf("[Sub] %s: dataType changed from %q to %q", e.id, old, dt)
}
...
func (e *StatementSub) updateConnectorTypes() {
	if e.wireMgr == nil {
		return
	}
...
func (e *StatementSub) IsHidden() bool { return e.hidden }

// SetHidden sets whether the label is hidden below the device.
func (e *StatementSub) SetHidden(h bool) {
	e.hidden = h
	go func() {
...
func (e *StatementSub) SetOnRemove(fn func(id string)) {
	e.onRemove = fn
}
...
func (e *StatementSub) SetGridAdjust(gridAdjust grid.Adjust) {
...
func (e *StatementSub) SetSize(width, height rulesDensity.Density) {
	e.width = width
	e.height = height
	if e.elem != nil {
...
func (e *StatementSub) SetHexMenu(m *mainMenu.SpriteHexMenu) {
	e.hexMenu = m
}
...
func (e *StatementSub) SetContextMenu(c *contextMenu.Controller) {
	e.ctxMenu = c
}
...
func (e *StatementSub) getBodyMenuItems() []contextMenu.Item {
...
func (e *StatementSub) settingsSubmenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementSub) getOutputMenuItems() []contextMenu.Item {
...
func (e *StatementSub) getOutputMonitorSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementSub) getOutputValueSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
func (e *StatementSub) getOutputFormatSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
...
// tutorial.
func (e *StatementSub) getOutputTutorialHexItems() []hexMenu.MenuItem {
...
func (e *StatementSub) getOutputTutorialSteps() []hexMenu.TutorialStep {
...
func (e *StatementSub) wireEvents() {
...
// RegisterConnectors registers all connection points of this StatementSub
// with the wire.Manager. Must be called AFTER Init() (when e.elem exists).
//
// Português:
//
//	RegisterConnectors registra todos os pontos de conexão deste StatementSub
//	no wire.Manager. Deve ser chamado APÓS Init() (quando e.elem existe).
func (e *StatementSub) RegisterConnectors() {
...
func (e *StatementSub) recacheOrnament() {
...
func (e *StatementSub) injectLabelIntoSvg(svgXml string, ornH rulesDensity.Density) string {
...
func (e *StatementSub) initWarningElement() {
...
func (e *StatementSub) updateWarningPosition() {
...
func (e *StatementSub) showInspectOverlay() {
...
func (e *StatementSub) GetInspectConfig() interface{} {
	hiddenVal := "false"
	if e.hidden {
		hiddenVal = "true"
	}
	...
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
			...
			},
			{
				...
				ContentURL: "/help/devices/math/statementSub.md", // todo: to rules
			},
			{
...
func (e *StatementSub) codePreview() string {
	code := ""
	if e.comment != "" {
		// Multi-line comments
...
// Implements scene.Inspectable.
func (e *StatementSub) ApplyProperties(values map[string]string) {
	changed := false

	if v, ok := values["label"]; ok && v != "" && v != e.label {
		e.label = v
		changed = true
		log.Printf("[Sub] %s: label set to %q", e.id, v)
	}

	if v, ok := values["comment"]; ok && v != e.comment {
		e.comment = v
		log.Printf("[Sub] %s: comment set to %q", e.id, v)
	}
	...
	if v, ok := values["hidden"]; ok {
		...
		if h != e.hidden {
			e.SetHidden(h)
			log.Printf("[Sub] %s: hidden set to %v", e.id, h)
		}
	}
...
func (e *StatementSub) GetProperties() map[string]interface{} {
...
func (e *StatementSub) GetDeviceType() string { return "StatementSub" }

func (e *StatementSub) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementSub) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementSub) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementSub) SetSceneNotify(fn func()) { e.sceneNotify = fn }

func (e *StatementSub) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
	x, y := e.elem.GetPosition()
