// devices/compMath/statementAdd.go
//
...
func (e *StatementAdd) SetStage(stage sprite.Stage) {
	e.stage = stage
}
...
func (e *StatementAdd) SetDataType(dt string) {
...
func (e *StatementAdd) updateConnectorTypes() {
	if e.wireMgr == nil {
		return
	}
...
func (e *StatementAdd) IsHidden() bool { return e.hidden }

// SetHidden sets whether the label is hidden below the device.
func (e *StatementAdd) SetHidden(h bool) {
	e.hidden = h
	go func() {
...
func (e *StatementAdd) SetOnRemove(fn func(id string)) {
	e.onRemove = fn
}
...
func (e *StatementAdd) SetGridAdjust(gridAdjust grid.Adjust) {
...
func (e *StatementAdd) SetSize(width, height rulesDensity.Density) {
	e.width = width
	e.height = height
	if e.elem != nil {
...
func (e *StatementAdd) SetHexMenu(m *mainMenu.SpriteHexMenu) {
	e.hexMenu = m
}
...
// Delete goes first (canonicalised order per decision D4), Inspect
// second, and the device-specific Settings submenu last.
//
// Settings is a body-level action because it changes the data type
// on every connector at once; it is not a per-connector decision.
//
// Português: Retorna itens do menu de contexto para clique no corpo.
// Ordem canonizada: Delete, Inspect, Settings.
func (e *StatementAdd) getBodyMenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		...
		}),
		{
			...
			ViewBox:         "0 0 512 512",
			HelpKey:         "helpMenuAddSettings",
			HelpFallback:    "Changes the data type of every connector at once. Choose Int, Float, or String — the inputs and the output all switch together so Add keeps its signature consistent.",
...
// the Settings item in the body menu. The linear renderer injects
// its own Back row automatically — the submenu does NOT need to
// include a manually-placed back button.
//
// Português: Submenu de seleção de tipo alcançado pelo Settings.
// O renderer linear injeta a linha "Back" automaticamente; o
// submenu NÃO precisa incluir um botão de voltar manual.
func (e *StatementAdd) settingsSubmenuItems() []contextMenu.Item {
	return []contextMenu.Item{
		{
			ID:              "typeInt",
			Label:           translate.T("menuTypeInt", "Int"),
			FontAwesomePath: rulesIcon.KFAPlus,
			ViewBox:         "0 0 448 512",
			HelpKey:         "helpMenuTypeInt",
			HelpFallback:    "Sets every connector on this device to integer. Integer values carry no fractional part.",
			OnClick:         func() { e.SetDataType("int") },
		},
		{
			ID:              "typeFloat",
			Label:           translate.T("menuTypeFloat", "Float"),
			FontAwesomePath: rulesIcon.KFADivide,
			ViewBox:         "0 0 448 512",
			HelpKey:         "helpMenuTypeFloat",
			HelpFallback:    "Sets every connector on this device to 32-bit floating point.",
			OnClick:         func() { e.SetDataType("float") },
		},
		{
			ID:              "typeString",
			Label:           translate.T("menuTypeString", "String"),
			FontAwesomePath: rulesIcon.KFABars,
			ViewBox:         "0 0 448 512",
			HelpKey:         "helpMenuTypeString",
			HelpFallback:    "Sets every connector on this device to string. The Add operator concatenates text values when strings are connected.",
			OnClick:         func() { e.SetDataType("string") },
		},
	}
}

// getInputXMenuItems returns the context menu for the inputX connector.
// ConnectorMenu already returns []contextMenu.Item after wireMenu.go
// was rewritten — inputs get just the Connect action, no submenu.
//
// Português: Menu de contexto do conector inputX. Apenas ação Connect.
func (e *StatementAdd) getInputXMenuItems() []contextMenu.Item {
	return mainMenu.ConnectorMenu(e.wireMgr, e.id, "inputX")
}

// getInputYMenuItems returns the context menu for the inputY connector.
//
// Português: Menu de contexto do conector inputY.
func (e *StatementAdd) getInputYMenuItems() []contextMenu.Item {
...
func (e *StatementAdd) getOutputMenuItems() []contextMenu.Item {
	return mainMenu.ConnectorMenu(e.wireMgr, e.id, "output",
		contextMenu.Item{
			...
			HelpFallback:    "Opens the output monitor. You can see the current value, its history, or configure how the value is formatted (decimal, hex, binary).",
			Submenu:         e.getOutputMonitorSubmenu(),
		},
	)
}

// getOutputMonitorSubmenu returns level 2 of the monitoring tree:
// Value (further submenu) and History (leaf action).
func (e *StatementAdd) getOutputMonitorSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
			ID:              "monValue",
			Label:           translate.T("menuMonitorValue", "Value"),
			FontAwesomePath: rulesIcon.KFAEye,
			ViewBox:         "0 0 512 512",
			HelpFallback:    "Inspect the live value of this output. The next level lets you choose number format or value range.",
			Submenu:         e.getOutputValueSubmenu(),
		},
		{
			ID:              "monHistory",
			Label:           translate.T("menuMonitorHistory", "History"),
			FontAwesomePath: rulesIcon.KFAClockRotateLeft,
			ViewBox:         "0 0 512 512",
			HelpFallback:    "Show the history of values that flowed through this output over time.",
			OnClick: func() {
				log.Printf("[CTXMENU] monitor output history: id=%v", e.id)
			},
		},
	}
}

// getOutputValueSubmenu returns level 3: Format (submenu) and Range.
func (e *StatementAdd) getOutputValueSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
			ID:              "valFormat",
			Label:           translate.T("menuValueFormat", "Format"),
			FontAwesomePath: rulesIcon.KFAGear,
			ViewBox:         "0 0 512 512",
			HelpFallback:    "Pick a numeric format for how the value is displayed in the monitor — decimal, hex, or binary.",
			Submenu:         e.getOutputFormatSubmenu(),
		},
		{
			ID:              "valRange",
			Label:           translate.T("menuValueRange", "Range"),
			FontAwesomePath: rulesIcon.KFAArrowsUpDownLeftRight,
			ViewBox:         "0 0 512 512",
			HelpFallback:    "Set an expected value range on this output. Useful for sparklines and scaling on the frontend dashboard.",
			OnClick: func() {
				log.Printf("[CTXMENU] output value range: id=%v", e.id)
			},
		},
	}
}
...
// All three are leaf actions — this is the deepest the normal menu
// goes. Good stress test for the linear renderer's back-navigation.
func (e *StatementAdd) getOutputFormatSubmenu() []contextMenu.Item {
	return []contextMenu.Item{
		{
			ID:              "fmtDecimal",
			Label:           translate.T("menuFormatDecimal", "Decimal"),
			FontAwesomePath: rulesIcon.KFAPlus,
			ViewBox:         "0 0 448 512",
			HelpFallback:    "Show this value as a base-10 decimal number.",
			OnClick:         func() { log.Printf("[CTXMENU] output format = Decimal: id=%v", e.id) },
		},
		{
			ID:              "fmtHex",
			Label:           translate.T("menuFormatHex", "Hex"),
			FontAwesomePath: rulesIcon.KFABars,
			ViewBox:         "0 0 448 512",
			HelpFallback:    "Show this value as a base-16 hexadecimal number, handy for byte-level debugging.",
			OnClick:         func() { log.Printf("[CTXMENU] output format = Hex: id=%v", e.id) },
		},
		{
			ID:              "fmtBinary",
			Label:           translate.T("menuFormatBinary", "Binary"),
			FontAwesomePath: rulesIcon.KFADivide,
			ViewBox:         "0 0 448 512",
			HelpFallback:    "Show this value as a base-2 binary number — one character per bit.",
...
// RegisterConnectors registers all connection points of this StatementAdd
// with the wire.Manager. Must be called AFTER Init() (when e.elem exists).
//
// Português:
//
//	RegisterConnectors registra todos os pontos de conexão deste StatementAdd
//	no wire.Manager. Deve ser chamado APÓS Init() (quando e.elem existe).
func (e *StatementAdd) RegisterConnectors() {
...
func (e *StatementAdd) recacheOrnament() {
...
func (e *StatementAdd) injectLabelIntoSvg(svgXml string, ornH rulesDensity.Density) string {
...
func (e *StatementAdd) initWarningElement() {
...
func (e *StatementAdd) updateWarningPosition() {
...
func (e *StatementAdd) showInspectOverlay() {
...
func (e *StatementAdd) GetInspectConfig() interface{} {
	hiddenVal := "false"
	if e.hidden {
		hiddenVal = "true"
	}
	...
	return overlay.Config{
		...
		Tabs: []overlay.Tab{
			...
			},
			{
				...
				ContentURL: "/help/devices/math/statementAdd.md", // todo: to rules
			},
			{
...
func (e *StatementAdd) codePreview() string {
	code := ""
	if e.comment != "" {
		// Multi-line comments
...
// Implements scene.Inspectable.
func (e *StatementAdd) ApplyProperties(values map[string]string) {
	changed := false
...
func (e *StatementAdd) GetProperties() map[string]interface{} {
...
func (e *StatementAdd) GetDeviceType() string { return "StatementAdd" }

func (e *StatementAdd) GetOuterBBox() scene.Rect {
	if e.elem == nil {
		return scene.Rect{}
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementAdd) GetInnerBBox() *scene.Rect {
	if e.elem == nil {
		return nil
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementAdd) GetKind() scenegraph.Kind { return scenegraph.KindSimple }
func (e *StatementAdd) SetSceneNotify(fn func()) { e.sceneNotify = fn }

// MoveBy shifts the device by (dx, dy) and updates all side effects.
// Implements scene.Movable — used by container drag logic.
func (e *StatementAdd) MoveBy(dx, dy float64) {
	if e.elem == nil {
		return
	}
	x, y := e.elem.GetPosition()
...
func (e *StatementAdd) OpenInspect() { go e.showInspectOverlay() }
