...
export async function feedRate(projectId, rating) {
    if (!S.user) { toast('warning', 'Faça login para avaliar projetos.'); return; }
...
export async function feedToggleFollow(projectId, authorUsername, currentlyFollowing) {
    if (!S.user) { toast('warning', 'Faça login para seguir usuários.'); return; }
...
export async function feedSubmitComment(projectId) {
    if (!S.user) { toast('warning', 'Faça login para comentar.'); return; }
...
export async function feedDeleteComment(projectId, commentId) {
    if (!await showConfirm('Excluir este comentário?', 'Excluir', 'Cancelar')) return;
...
export async function feedOpenReport(projectId) {
    if (!S.user) { toast('warning', 'Faça login para reportar.'); return; }
